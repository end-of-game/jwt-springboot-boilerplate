...
export class ConnectService {
  ...
  private _connectSource: BehaviorSubject<Connect> = new BehaviorSubject<
    Connect
  >(new Connect());
  ...
  private getUserInformation(): void {
    this.httpService
      ...
      .subscribe(
        data => {
          this._connectSource.next(
            new Connect(data.username, '', data.email, data.authorities)
          );
        },
        err => {
          console.log(err);
  ...
  login(username: string, password: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.httpService
        .post(this.BASE_URL + '/auth', JSON.stringify({ username, password }))
        .pipe(map(res => res.json()))
        .subscribe(
          data => {
